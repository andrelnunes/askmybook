...
def construct_prompt(question: str, context_embeddings: dict, df: pd.DataFrame) -> tuple[str, str]:
    ...
    header = """Alfredo Soares, fundador da XTECH COMMERCE, que no final de 2017, foi comprada pela VTEX, maior empresa da América Latina de tecnologia de e-commerce. Também fundou a SocialRocket, plataforma de automação para Instagram que transforma perfis em canais de vendas. Com ela, em dois anos Alfredo e sua equipe chegaram a mais de 150 mil usuários cadastrados em 7 países. A obra de Alfredo Soares pode ser chamada de “um guia para o empreendedor que põe a mão na massa e vende”.\n"""

    question_1 = "\n\n\nQ: Vender é só para o time comercial?\n\nA: Levando em consideração que vender é gerar valor para o seu cliente, apresentando a solução que melhor atende suas necessidades, é antiquado pensar que vender é responsabilidade somente dos executivos de vendas. As empresas que desejam se destacar no mercado e construir uma verdadeira máquina de vendas precisam fazer com que toda a empresa trabalhe focada em encantar os consumidores, colocando eles no centro do processo, o que chamamos de negócios customer centric."
    ...
    return (header + "".join(chosen_sections) + question_1 + question_2 + question_3 + "\n\n\nQ: " + question + "\n\nA: "), ("".join(chosen_sections))
...
def index(request):
    return render(request, "index.html", { "default_question": "Qual o assunto deste livro?" })
